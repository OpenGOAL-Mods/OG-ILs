"""Loopback-only HTTP API and dashboard for OpenGOAL replay files."""
...
<section class="card"><h2>Replays</h2><table><thead><tr><th>Name</th><th>Mission</th><th>Time</th><th>Player ID</th><th>Replay ID</th><th>SRC runner</th><th>Speedrun.com</th><th></th></tr></thead><tbody id="replays"></tbody></table></section></main><script>
let state={replays:[],players:[],runners:[],moderator:{},settings:{}};
async function api(path,options={}){options.headers={'Content-Type':'application/json',...(options.headers||{})};let r=await fetch(path,options);let data=await r.json().catch(()=>({}));if(!r.ok)throw Error(data.error||r.statusText);return data}
...
document.querySelector('#auto-submit').onchange=async e=>{await api('/api/settings',{method:'PATCH',body:JSON.stringify({auto_submit:e.target.checked})});await refresh()};refresh();setInterval(refresh,5000);</script></body></html>"""
...
class ReplayRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            elif parts == ["api", "state"]: self._json(HTTPStatus.OK, self.store.public_state())
    ...
    def do_POST(self) -> None:
        try:
            parts, data = self._parts(), self._body()
...
class ReplayHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(self, address: tuple[str, int], store: ReplayStore) -> None:
        super().__init__(address, ReplayRequestHandler); self.store = store


def main() -> None:
    parser = argparse.ArgumentParser(description="OpenGOAL local replay server")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT); parser.add_argument("--no-browser", action="store_true")
    args = parser.parse_args(); store = ReplayStore(); server = ReplayHTTPServer((DEFAULT_HOST, args.port), store)
    url = f"http://{DEFAULT_HOST}:{args.port}/"; print(f"OpenGOAL Replay Server is running at {url}"); print(f"Replay data: {store.root}")
    if not args.no_browser: threading.Timer(0.35, webbrowser.open, args=(url,)).start()
